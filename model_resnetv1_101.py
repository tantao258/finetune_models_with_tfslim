    ...
    def __init__(self, num_classes, train_layers=None, learning_rate=0.001, model="train", weights_path='DEFAULT'):
        ...
        if model == "train" or model == "val":
            with arg_scope(resnet_v1.resnet_arg_scope()):
                self.logits, _ = resnet_v1.resnet_v1_101(self.x_input, num_classes=num_classes, is_training=True)

        if model == "test":
            with arg_scope(resnet_v1.resnet_arg_scope()):
                self.logits, _ = resnet_v1.resnet_v1_101(self.x_input, num_classes=num_classes, is_training=False)
