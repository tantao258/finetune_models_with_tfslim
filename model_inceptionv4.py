    ...
    def __init__(self, num_classes, train_layers=None, learning_rate=0.001, model="train", weights_path='DEFAULT'):
        ...
        if model == "train" or model == "val":
            with arg_scope(inception.inception_v4_arg_scope()):
                self.logits, _ = inception.inception_v4(self.x_input, num_classes=num_classes, is_training=True)

        if model == "test":
            with arg_scope(inception.inception_v4_arg_scope()):
                self.logits, _ = inception.inception_v4(self.x_input, num_classes=num_classes, is_training=False)
