...
class ResNetv2_152(object):
    def __init__(self, num_classes, train_layers=None, learning_rate=0.001, model="train", weights_path='DEFAULT'):
        ...
        """Create the graph of the resnetv2_152 model.
        """

        # Parse input arguments into class variables
        if weights_path == 'DEFAULT':
            self.WEIGHTS_PATH = "./pre_trained_models/resnet_v2_152.ckpt"
        else:
            self.WEIGHTS_PATH = weights_path
        self.train_layers = train_layers

        with tf.variable_scope("input"):
            self.image_size = resnet_v2.resnet_v2_152.default_image_size
        ...
        if model == "train" or model == "val":
            with arg_scope(resnet_v2.resnet_arg_scope()):
                self.logits, _ = resnet_v2.resnet_v2_152(self.x_input, num_classes=num_classes, is_training=True)
