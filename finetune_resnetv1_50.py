...
from model_resnetv1_50 import ResNetv1_50
...
with tf.Session() as sess:
    ...
    out_dir = os.path.abspath(os.path.join(os.path.curdir, "runs", "resnetv1_50", timestamp))
    print("Writing to {}\n".format(out_dir))

    # define summary
    grad_summaries = []
    for g, v in resnetv1_50.grads_and_vars:
        if g is not None:
    ...
    loss_summary = tf.summary.scalar("loss", resnetv1_50.loss)
    acc_summary = tf.summary.scalar("accuracy", resnetv1_50.accuracy)
    ...
    while True:
        ...
        # validation
        current_step = tf.train.global_step(sess, resnetv1_50.global_step)
